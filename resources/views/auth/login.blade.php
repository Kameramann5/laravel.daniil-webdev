...
<head>
    ...
    <style>
        ...
        *,::after,::before{box-sizing:border-box;border-width:0;border-style:solid;border-color:#e5e7eb}::after,::before{--tw-content:''}
        ...
        /* ...сокращено, полную стиль-копию возьми из welcome, тут начало... */
        ...
        .bg-green-100{background-color:#dcfce7}.border-l-4{border-left-width:4px}
        .border-green-400{border-color:#4ade80}.p-4{padding:1rem}.mb-4{margin-bottom:1rem}.text-green-700{color:#15803d}
        ...
        .rounded{border-radius:0.25rem}.px-3{padding-left:0.75rem;padding-right:0.75rem}
...
<body class="antialiased">
    <div class="relative flex justify-center items-center min-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
        <div class="bg-white dark:bg-gray-800/50 rounded-lg shadow-2xl p-8 w-full max-w-md mx-auto">
            <h1 class="text-2xl font-bold mb-6 text-center text-gray-900 dark:text-white">Вход на сайт</h1>
            @if(session('success'))
                <div class="bg-green-100 border-l-4 border-green-400 p-4 mb-4 text-green-700">
                    {{ session('success') }}
                </div>
            @endif
            @if ($errors->any())
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <button class="w-full bg-red-500 text-white py-2 rounded hover:bg-red-600 transition">Войти</button>
            </form>
            <div class="mt-4 text-center">
                Нет аккаунта?
                <a href="{{ route('register') }}" class="underline text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white">Зарегистрируйтесь</a>
            </div>
        </div>
    </div>
</body>
